        ...
        for i in range(0, len(els)):  # пробежка по кнопкам
            if type(els[i]) == tuple:  # если здесь тип tuple
                if (start_watching <= els[i][0] < end_watching) and (1 not in dict.values(a)):  # (старт <= поз < финиш) и (ни одна лампа еще не включена)
                    ...
                    a[els[i][1]] = 0  # в словарь этой лампы ставлю отметку что она выключена
                print(f"1 что в итоге творится в словаре с лампами: {a}, какое время записано для калькуляции: {b}, сколько же у нас светились лампы:{d}")
            else:  # здесь объект datetime
                if (start_watching <= els[i] < end_watching) and (1 not in dict.values(a)):  # (старт <= поз < финиш) и (ни одна лампа еще не включена)
                    print("это должа быть первая лампа на дататайме, ее нужно будит зажечь в словарь")
                    ...
                    if c.count("1") == 0:  # если все лампы выключились
                        d += (els[i] - b).total_seconds()  # записываем результат в секундах
                elif els[i] < start_watching and a[els[i]] == 1:  # если (поз < старт) и (эта лампа включена)
                    a[els[i]] = 0  # в словарь этой лампы ставлю отметку что она выключена
                print(f"2 что в итоге творится в словаре с лампами: {a}, какое время записано для калькуляции: {b}, сколько же у нас светились лампы:{d}")
        print(f"3 что в итоге творится в словаре с лампами: {a}, какое время записано для калькуляции: {b}, сколько же у нас светились лампы:{d}")
        return int(d)
